...
class RGBD_FPN(Backbone):
    ...
    def forward(self, x):
        ...
        raw_laterals = [prev_features_tmp.clone()]
        ...
        for features, lateral_conv, output_conv in zip(x[1:], self.lateral_convs[1:],  self.output_convs[1:]):
            ...
            raw_laterals.insert(0, lateral_features.clone())
        ...
        if self.top_block is not None:
            ...
            results.extend(self.top_block(top_block_in_feature))
        assert len(self._out_features) == len(results)
        return dict(zip(self._out_features, results)), dict(zip(self._out_features, raw_laterals))
